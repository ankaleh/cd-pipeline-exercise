...
blogsRouter.get('/', async (request, response) => {
  const blogs = await Blog.find({}).populate('user', { username: 1, nimi: 1 }).populate('comments', { text: 1 })
...
blogsRouter.post('/', async (request, response) => {
  ...
  const decodedToken = jwt.verify(request.token, process.env.SECRET) //Varmistetaan metodilla verify tokenin oikeellisuus ja dekoodataan se. Olion decodedToken sisällä nyt kirjautuneen käyttäjän username ja id.
